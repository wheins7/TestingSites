...
// Name: Maxwell Murray
// Email: msmurray4@wisc.edu
// Team: DG
// TA: Yelun
// Lecturer: Gary Dahl
// Notes to Grader: Passed initial tests in gradescope but was very slow with this data so needed to build off of griffin's
...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
         ...
         * vertices in this graph: start and end. This path contains an ordered list of the data
         * within each node on this path, and also the distance or cost of all edges that are a part
         * of this path.
         * 
        ...
        protected Path dijkstrasShortestPath(T start, T end) {
          if (!containsVertex(start) || !containsVertex(end)
              || !containsPathToVertex(vertices.get(end))) {
            throw new NoSuchElementException();
          }
          Vertex begin = vertices.get(start);
          Vertex target = vertices.get(end);
          LinkedList<Vertex> done = new LinkedList<>();
          PriorityQueue<Path> horizon = new PriorityQueue<Path>();
          Path initial = new Path(begin);
          horizon.add(initial);
          while (!horizon.isEmpty()) {
            Path now = horizon.poll();
            if (now.end.equals(target)) {
              return now;
            }
            done.add(now.end);
            LinkedList<Edge> successors = now.end.edgesLeaving;
            for (Edge x : successors) {
              Vertex check = vertices.get(x);
              if (!done.contains(check)) {
                Path copy = new Path(now);
                copy.extend(x);
                horizon.add(copy);
              }
            }
        ...
        /**
         * @author Griffin, smart idea to see if there truly is a path between nodes
         * 
         * private helper method to check to see if the end vertex has an edge that points to the
         * vertex
         * 
        ...
        private boolean containsPathToVertex(Vertex v) {
          ...
          while (elements.hasMoreElements()) {
            LinkedList<Edge> outgoing = elements.nextElement().edgesLeaving; // get the outgoing
                                                                             // edges
