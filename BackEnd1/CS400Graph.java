...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
        	...
        	//create the new data sequence 
        	this.dataSequence = new LinkedList<>();
        	//add the element from the previous list one by one
        	for(T ele: source.dataSequence) {
        		this.dataSequence.add(ele);
        	}
        	this.end = source.end;
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
    	if(!containsVertex(start)||!containsVertex(end)||!containsPathToVertex(vertices.get(end))) {
    		throw new NoSuchElementException(); //check to see if vertex is contained
    	}
    	LinkedList<Vertex> visited = new LinkedList<Vertex>(); //array of visited nodes
    	Vertex source = vertices.get(start); //start vertex
    	Path begin = new Path(source); //shortest path
    	PriorityQueue<Path> pq = new PriorityQueue<Path>(); //priority queue of paths
    	pq.add(begin); //add shortest path
    	while(!pq.isEmpty()) { //while the priority queue isn't empty
    		Path C = pq.remove(); //C is equal to the first element in the pq
    		...
    		for(CS400Graph<T>.Edge a: C.end.edgesLeaving) {
    			//copy the path to a new path
    			Path updated=new Path(C);
    			//extend the new path
    			updated.extend(a);
    			//add it back 
    			pq.add(updated);
	...
	throw new NoSuchElementException(); //if the value is not returned in the loop then there is no shortest path
    }
    /**
