...
// Name: William Heins & Max Murray
// Email: wheins@wisc.edu & msmurray4@wisc.edu
// Team: DG
// TA: Yelun Bao
// Lecturer: Florian Heimler
// Notes to Grader:
...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
    	if(!containsVertex(start)||!containsVertex(end)||!containsPathToVertex(vertices.get(end))) {
    		throw new NoSuchElementException("vertex not found"); //check to see if vertex is contained
    	}
    	LinkedList<Vertex> visited = new LinkedList<Vertex>(); //array of visited nodes
    	Vertex startV = vertices.get(start); //start vertex
    	Path shortestPath = new Path(startV); //shortest path
    	PriorityQueue<Path> pq = new PriorityQueue<Path>(); //priority queue of paths
    	pq.add(shortestPath); //add shortest path
    	while(!pq.isEmpty()) { //while the priority queue isn't empty
    		Path C = pq.peek(); //C is equal to the first element in the pq
    		pq.remove();
    		if(C.end.equals(vertices.get(end))) { //check to see if the vertex of the end of the path is equal to the end
    			return C;  //if it is equal to the end then it is the shortest path to that node
    		}
    		visited.add(C.end); //mark the vertex as visited
    		LinkedList<Edge> successors = C.end.edgesLeaving; //get the list of the successors of the vertex
    		for(int i=0;i<successors.size();i++) {
    			Edge cEdge = successors.get(i); //get the outgoing edge
    			Vertex S = vertices.get(successors.get(i)); //get the vertex of the successor
    			if(!visited.contains(S)) {  //check to see if the successor has been visited
    				Path newPath = new Path(C); //copy path of the predecessor
    				newPath.extend(cEdge); //add edge to the newPath
    				pq.add(newPath); // add the newPath to the priority queue
    			}
    		}
    	}
	throw new NoSuchElementException("no path found to the vertex"); //if the value is not returned in the loop then there is no shortest path
    }
    /**
     * private helper method to check to see if the end vertex has an edge that points to the vertex
     * @param v vertex that is checked
     * @return true if there is a path to the vertex
     */
    private boolean containsPathToVertex(Vertex v) {
    	boolean returnVal = false;
    	Enumeration<Vertex> elements = vertices.elements(); //get enumeration of all the vertexes
    	while(elements.hasMoreElements()) {
    		LinkedList<Edge> outgoing = elements.nextElement().edgesLeaving; //get the outgoing edges
    		for(int i=0;i<outgoing.size();i++) {
    			if(outgoing.get(i).target.equals(v)) { //if one of the targets is equal to the v
