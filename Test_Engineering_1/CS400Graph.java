...
// TA: Yelun
// Lecturer: Dahl
// Notes to Grader: Edited to fit with 
...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		...
		// NoSuchElementException & check to see if vertex is contained
		...
		while (!pq.isEmpty()) { // while the priority queue isn't empty
			...
			pq.remove();
			if (C.end.equals(vertices.get(end))) { // check to see if the vertex of the end of the path is equal to the
													// end
	...
	/**
	 * private helper method to check to see if the end vertex has an edge that
	...
	// System.out.println(graph.shortestPath(2, 0).toString());
	// System.out.println(graph.getPathCost(0, 6));
